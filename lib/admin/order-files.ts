...
export const ADMIN_ORDER_PRIVATE_FILE_FULFILLMENT_KINDS = [
  "documentation",
  "floor_plans",
  "full_package",
  "pdf_email_package"
...
export function getAdminOrderPrivateFileLabel(fileType: string, fallbackTitle = "") {
  ...
  return ADMIN_ORDER_PRIVATE_FILE_LABELS[normalizedType] || getProjectFileTypeConfig(normalizedType).label || fallbackTitle || fileType || "Plik prywatny";
}
...
function toPrivateFile(row: PrivateFileRow, project: ProjectRow): AdminOrderPrivateFile {
  ...
  return {
    ...
    autoSendAfterPayment: row.auto_send_after_payment === true,
    requiredForPublication: row.required_for_publication === true
  };
}
...
function fileMatchesKind(file: AdminOrderPrivateFile, kind: AdminOrderPrivateFileFulfillmentKind) {
  ...
  if (kind === "floor_plans") return fileType === "floor_plans" || fileType === "floor_plan" || path.includes("floor-plans");
...
export async function getAdminOrderPrivateFilesByProjectKey(items: AdminOrderProjectFileRef[]): Promise<AdminOrderPrivateFileLookup> {
  ...
  async function collectProjectsBy(field: "code" | "slug", values: string[]) {
    ...
    const { data, error } = await supabaseClient.from("projects").select("id, code, slug, name").in(field, values);
    if (error) {
  ...
  const { data: fileRows, error: filesError } = await supabaseClient
    .from("project_files")
    .select("id, project_id, bucket, file_type, title, path, version, active, auto_send_after_payment, required_for_publication, created_at")
  ...
  for (const item of items) {
    const project = projectsByCode.get(normalizeKey(item.projectCode)) || projectsBySlug.get(normalizeKey(item.projectSlug));
    if (!project) continue;
